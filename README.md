...
Готовая к развёртыванию первая версия коммерческого сайта детейлинг-центра: адаптивная публичная часть, каталог услуг, запись на первичный осмотр по свободным часовым слотам, защищённая админ-панель, PostgreSQL, гарантированная доставка Telegram-уведомлений через outbox и Docker-инфраструктура.
...
- `GET /api/bookings/availability?date=YYYY-MM-DD` — свободные часы для первичного осмотра;
- `POST /api/bookings` — создать подтверждённую запись на осмотр с одной или несколькими услугами;
...
- `GET /api/admin/bookings/export.xlsx` — защищённая XLSX-выгрузка осмотров и сводной статистики;
...
- Текущий календарь резервирует один общий час первичного осмотра. Выбранные услуги нужны для подготовки менеджера и не резервируют время выполнения работ; рабочий заказ и производственное расписание следует реализовать отдельными сущностями.
